...
gantry_list = ['G009233001000810020',
               ...
               'G009233001000720010',
               'G009233001001020020']
df2 = pd.read_csv(r'config_all.csv', encoding="utf-8")
df3 = pd.read_csv(r'mileage_hundred(pxy).csv',encoding="utf-8")


def generate_historyRou(startTime, endTime):
    ...
    df1['startTime'] = [clean_time(x, H) for x in df1['startTime']]
    df1['endTime'] = [clean_time(x, H) for x in df1['endTime']]
    df1 = df1[[x in gantry_list for x in df1['originEdge']]]
    df1 = df1[[x in gantry_list for x in df1['destinationEdge']]]
    ...
    def make_Rouxml():
        ...
        # 保存
        out_path = r"C:\Users\pengx\Desktop\高信history1128\new.rou.xml"
        fp = open(out_path, 'w', encoding="utf-8")
        xml.writexml(fp, indent='  ', addindent='  ', newl='\n')

    def make_cfg_xml():

        start = datetime.datetime.strptime(startTime, '%Y-%m-%d %H:%M:%S')  # .strftime('%H:%M:%S')
        end = datetime.datetime.strptime(endTime, '%Y-%m-%d %H:%M:%S')  # .strftime('%H:%M:%S')
        t = int((end.hour - start.hour) * 3600) + int((end.minute - start.minute) * 60) + int(
            (end.second - start.second))

        xml_path = r"test.sumocfg"
        dom = xml.dom.minidom.parse(xml_path)  # dom算是指代xml文件的对象 xml_path为xml的存储路径
        time = dom.getElementsByTagName("end")  # hostdev1是标签为hostdev的节点列表
        time[0].setAttribute('value', str(t))  # 将属性type的值更改为pciiiii
...
test
'''
#run("2022-11-28 08:00:00", "2022-11-28 09:00:00")
